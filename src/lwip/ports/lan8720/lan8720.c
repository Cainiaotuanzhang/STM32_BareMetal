...
u8 ETH_MACDMA_Config(void)
{
    ...
    ETH_DeInit();                               //复位MAC
    ETH_SoftwareReset();                        //MAC DMA控制器软件重启
    while (ETH_GetSoftwareResetStatus() == SET);//等待MAC DMA控制器软件重启完成

    ETH_StructInit(&ETH_InitStructure);         //设置ETH MAC为默认值
    //ETH MAC非默认值参数的设置
...
u8 LAN8720_Init(void)
{
    ...
    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA|RCC_AHB1Periph_GPIOC|RCC_AHB1Periph_GPIOG|RCC_AHB1Periph_GPIOD,
    ...
    GPIO_PinAFConfig(GPIOA, GPIO_PinSource1, GPIO_AF_ETH); //引脚复用到ETH接口上
    ...
    GPIO_PinAFConfig(GPIOC, GPIO_PinSource1, GPIO_AF_ETH); //引脚复用到ETH接口上
    ...
    GPIO_PinAFConfig(GPIOG, GPIO_PinSource11, GPIO_AF_ETH); //引脚复用到ETH接口上
    ...
    delay_ms(50);
    LAN8720_RST = 1;                //复位结束
    ETHERNET_NVICConfiguration();   //配置中断
